...
export default function EditorPage() {
  const [projects, setProjects] = useState<Project[]>([
    { id: "1", name: "Project Omega", slug: "project-omega", isOwner: true },
    { id: "2", name: "Microservices Mesh", slug: "microservices-mesh", isOwner: true },
    { id: "3", name: "Collaborative API", slug: "collaborative-api", isOwner: false }
  ])
  ...
  const [logs, setLogs] = useState([
    ...
    { id: "2", time: "15:42:15", user: "system", msg: "Ready to select or create a project workspace." }
  ])
  ...
  const handleRenameProject = (id: string, name: string) => {
    ...
    setProjects((prev) =>
      prev.map((p) => (p.id === id ? { ...p, name, slug: newSlug } : p))
    )
  ...
  return (
    <main className="flex h-screen flex-col bg-base font-sans text-copy-primary antialiased overflow-hidden">
      ...
      {!activeProject ? (
        ...
        <div className="flex flex-1 flex-col lg:flex-row overflow-hidden animate-in fade-in duration-300">
          ...
          <section className="flex-1 bg-[#080809] relative p-6 flex items-center justify-center overflow-hidden border-b lg:border-b-0 border-default select-none">
            ...
            <div className="relative w-full max-w-2xl aspect-[16/10] bg-surface/10 border border-default rounded-3xl p-6 shadow-2xl flex flex-col justify-between backdrop-blur-xs">
              {/* Active Project Title overlay on canvas */}
              <div className="absolute top-4 left-4 z-10 flex items-center gap-2">
                <span className="text-xs font-mono font-bold text-brand uppercase tracking-widest bg-brand-dim border border-brand/20 px-2.5 py-1 rounded-lg">
                  {activeProject.name}
                </span>
              </div>

              {/* Mock Nodes Group */}
              <div className="absolute inset-0 p-8 flex flex-col gap-6 justify-between pointer-events-none pt-16">
      ...
      <ProjectDialogs
        ...
        isLoading={isLoading}
        onClose={closeDialog}
        onCreateSubmit={(e) => handleCreateSubmit(e, handleCreateProject)}
        onRenameSubmit={(e) => handleRenameSubmit(e, handleRenameProject)}
        onDeleteConfirm={() => handleDeleteConfirm(handleDeleteProject)}
      />
    </main>
  )
}
